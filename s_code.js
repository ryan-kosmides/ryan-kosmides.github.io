/* Adobe Visitor ID v4.4.0 + AppMeasurement version: v2.17.0 (02/20/2020) */
/**
 * @license
...
function setIntegrateModuleForDI(propertyID, delay) {
	try {
		var maxDelay;		
		var diPropertyId;
		var eVar = 'eVar80';
		
		maxDelay = isNaN(delay) ? 750 : delay;
		diPropertyId = isNaN(propertyID) ? -1 : propertyID;	
		...
		console.log("Failed to load integrate module");
	}
}
/* end global shared functions */

function s_doPlugins(s) {
     //Disable eAgent, ARS, AARP
	 if(s.hasOwnProperty('channel')){
		if (s.channel == "eAgent" || s.channel == "AllstateRoadsideServices" || s.channel == "ARS/RoadSmart" || s.server == "www.aarproadside.com" ) {
			s.abort = true;
			console.log("Adobe Analytics aborted");
		}
	 }
	...
	if (pageURL) {
      ...
      var myRegExp2Omit = /\/eagent|alliance|agencygateway|agencyconnect|wps\/myportal|lxe/;
    ...
    /* Code version. */
    s.prop28 = "MCID4.4.0|AppMeasurement2.17.0|20200220";
  ...
  //  if (s.channel.toLowerCase().indexOf('aws') != -1) { s.eVar38 = s.channel; }
	...
	//Timestamp
	var unit = 1000 * 60 / 6;  //rounding to the nearest 10 second
	var date = new Date(); 
	var timeStamp = new Date(Math.round(date.getTime()/unit) * unit);
	s.eVar77 = timeStamp.toString().split(" GMT")[0];
...
/* 
*Plugin: setupLinkTrack 3.16 - DO NOT UPDATE. NEEDED FOR VIDEO TRACKING AUTOMATIC PROP12-14 
*/ 
...
/*
* Plugin Utility: apl v1.1 - DO NOT UPDATE. NEEDED FOR VIDEO TRACKING AUTOMATIC PROP12-14
*/
